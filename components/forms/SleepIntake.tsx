...
import { addSleepIntake, fetchDailySleep, deleteSleepIntake } from "@/lib/actions/sleepIntake.actions";
...
function AddSleepIntake({ userId }: { userId: string }) {
  ...
  const fetchSleepIntake = async () => {
    try {
      ...
      setDailySleep(sleepIntake.totalIntake);
  ...
  return (
    <div>
      ...
      <p className="text-white intakeText">
        Added sleep intake for today: {dailySleep} hours
      </p>
